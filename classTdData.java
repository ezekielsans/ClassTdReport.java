...
//        hard-coded account_code
    ...
    public String queryGenerator(String accountCodeParam, String selectedBatchParam) {
        ...
        query = "SELECT x.row_number, " //0
                ...
                + "cls.ctdNo, "
                + "cls.primary "
                + "FROM (SELECT DISTINCT ON (apd.acctno) sl.td_sl_no AS slNo, "
...
//                    + "ORDER BY cls.acctno";
    ...
    public void export0() {
        ...
        for (int i = 0; i < getAccountsWithSubsidiaryData().getAccountCodesFiltered().size(); i++) {
            ...
            cell = headerRow.createCell(columnNo++);
            cell.setCellValue("Primary Holder");
            cell.setCellStyle(boldStyle);
            ...
            for (int ii = 0; ii < getClassTdSummary().get(i).size(); ii++) {
                ...
                System.out.println("CHEKING IF NULL: " + cellD);
                if (!cellD.isEmpty()) {
                ...
                try {  //acctname
                    String cellF = ((String) getClassTdSummary().get(i).get(ii)[4].toString());
                    System.out.println("CHEKING IF NULL: " + cellF);
                    if (cellF.isEmpty()) {
                        dataRow.createCell(columnNo++).setCellValue(cellF);
                    } else {

                        dataRow.createCell(columnNo++).setCellValue("-");
                    }
                } catch (Exception e) {
                    System.out.println("Null @ String cellF");
                    e.getMessage();
                }

                //ctd no.
                Integer ctdNo = ((Integer) getClassTdSummary().get(i).get(ii)[9]);
                System.out.println("CHEKING IF NULL: " + ctdNo);
                if (ctdNo != null) {
                ...
                System.out.println("CHEKING IF NULL: " + cellBigD);
                if (cellBigD != null) {
                    cell.setCellValue(cellBigD.doubleValue());
                } else {
                    cell.setCellValue("-");
                }

                //check html get index value of primary holder                 
